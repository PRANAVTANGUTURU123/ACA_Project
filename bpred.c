             ...
             char *btb_repl)
{
  ...
  case BPred2bit:
	...
	fatal("cannot allocate BTB");
      /* Allocate adaptive miss counters */
      pred->btb.miss_count = calloc(btb_sets, sizeof(unsigned int));
...
if (pbtb)
{
  ...
  if (pbtb->addr == baddr)
  {
    ...
    /* ✅ Adaptive replacement: reset miss counter on success */
    if (pred->btb.repl == BPredRepl_Adaptive)
    {
      unsigned int set_idx = (baddr >> MD_BR_SHIFT) & (pred->btb.sets - 1);
      pred->btb.miss_count[set_idx] = 0;  /* reset after successful hit */
    }
  }
  else
  {
